...
RSpec.describe MinHeapTree, type: Class do
  ...
  before do
    tree.insert(tree.head, root)
  end

  describe "#insert(data)" do
    it "properly inserts a new node as head" do
      tree.insert(tree.head, martian)
      expect(tree.head.title).to eq "The Martian"
    end
    ...
    it "properly inserts a new node as root node" do
      ...
      expect(tree.head.title).to eq "District 9"
      expect(tree.head.left.title).to eq "The Matrix"
    end
    ...
    it "properly inserts a new node as root node and keeps heap structure" do
      ...
      expect(tree.head.title).to eq "The Martian"
      expect(tree.head.left.title).to eq "Inception"
      expect(tree.head.right.title).to eq "The Matrix"
    end
